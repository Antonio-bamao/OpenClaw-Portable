...
class DeliveryGateTests(unittest.TestCase):
    def test_gate_passes_local_checks_and_marks_external_evidence_pending(self) -> None:
        ...
        try:
            ...
            public_key_b64 = self._write_minimal_package(package_root, version="v2026.04.3")
    ...
    def test_gate_fails_when_package_signature_does_not_match_manifest(self) -> None:
        temp_dir = make_workspace_temp_dir()
        try:
            package_root = temp_dir / "OpenClaw-Portable"
            release_dir = temp_dir / "release"
            public_key_b64 = self._write_minimal_package(package_root, version="v2026.04.3")
            write_file(package_root / "README.txt", "changed after signing")
            write_update_manifest(package_root, entry_names=DEFAULT_UPDATE_ENTRY_NAMES)
            release_dir.mkdir(parents=True, exist_ok=True)
            with zipfile.ZipFile(release_dir / "OpenClaw-Portable-v2026.04.3.zip", "w") as archive:
                archive.writestr("OpenClaw-Portable/version.json", json.dumps({"version": "v2026.04.3"}))
                archive.writestr("OpenClaw-Portable/update-manifest.json", (package_root / "update-manifest.json").read_text(encoding="utf-8"))
                archive.writestr("OpenClaw-Portable/update-signature.json", (package_root / "update-signature.json").read_text(encoding="utf-8"))
            write_file(
                release_dir / "update.json",
                json.dumps(
                    {
            ...
            self.assertEqual(result.status, "failed")
            self.assertEqual(checks["release-assets"].status, "failed")
            self.assertIn("数字签名校验失败", checks["release-assets"].message)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)

    def test_gate_fails_when_update_json_points_to_wrong_package_url(self) -> None:
        temp_dir = make_workspace_temp_dir()
        try:
            package_root = temp_dir / "OpenClaw-Portable"
            release_dir = temp_dir / "release"
            ...
            self.assertEqual(result.status, "failed")
            self.assertEqual(checks["release-assets"].status, "failed")
            self.assertIn("packageUrl", checks["release-assets"].message)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
    ...
    def _write_minimal_package(self, package_root: Path, *, version: str) -> str:
        write_file(package_root / "OpenClawLauncher.exe", "exe")
        write_file(package_root / "_internal" / "base_library.zip", "python")
        ...
        write_file(package_root / "README.txt", "readme")
        write_update_manifest(package_root, entry_names=DEFAULT_UPDATE_ENTRY_NAMES)
