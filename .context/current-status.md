...
- 当前阶段：`v2026.05.3` 源码侧已定位并修复微信外部 npm 插件 register 失败的根因；用户在 `2026-05-08` 真实复测后确认微信可用。随后新暴露的 Feishu runtime config 问题也已在源码和当前 dist 状态中处理：缺少 Feishu 插件时不再写入 `channels.feishu`，旧 `channels.feishu` 已从当前 dist runtime config 删除。
- 今天已做：修过微信 QR 登录后的真实 `accountId` 投影、确认扫码后重启投影、运行中渠道变更先停 gateway 再重新投影并启动、provider patch 强制 `gateway.mode=local`、release 打包时保留完整 `runtime/openclaw/package.json`，并新增 `scripts/verify-wechat-plugin-runtime.py` 作为“已安装外部 npm 微信插件后可注册”的可重复 smoke。
- 根因确认：当前失败不是 `openclaw/plugin-sdk/*` import 本身，而是 packaged `runtime/openclaw/package.json` 曾被过度精简成只含 `./plugin-sdk/*` 的 shim；OpenClaw loader 只能通过 `package.json.name == "openclaw"` 定位宿主根目录，定位失败后找不到 `dist/plugins/runtime/index.js`，于是微信插件 register 报 `Unable to resolve plugin runtime module`。
- Feishu 新根因：OpenClaw `2026.5.6` 当前包不含 bundled Feishu 扩展，Feishu 已属于外部插件路线；旧 launcher 仍把保存过的飞书凭据投影为 `channels.feishu`，导致 OpenClaw 配置校验报 `unknown channel id: feishu` 并阻断 gateway 启动。
- 最新验证：`python -m unittest discover -s tests` 通过 `311` 项测试；`scripts/build-release-assets.ps1` 已重新生成 `dist/release/OpenClaw-Portable-v2026.05.3.zip` 与 `update.json`；release zip 检查确认没有恢复后的 `state/npm`、`state/runtime`、`state/channels`；恢复本机 state 后 `python scripts\verify-wechat-plugin-runtime.py --package-root dist\OpenClaw-Portable --timeout-seconds 45 --post-ready-wait-seconds 3 --output tmp\wechat-plugin-runtime-smoke-repack.json` 返回 `ok=true`。
- 防复发重点：下一版给用户的 zip 必须重新打包，不能复用旧 release zip；打包后必须同时跑 `python scripts\verify-wechat-plugin-runtime.py --package-root dist\OpenClaw-Portable` 和 Feishu 缺插件回归相关单测。任何 launcher-first 渠道投影前必须先确认 runtime 插件存在，缺插件时删除旧 runtime config key。
- 上下文维护：`work-log.md` 已超过 400 行，后续流水写入 `work-log（2）.md`；微信和 Feishu 两次异常都已在 `bug-log.md` 更新。
