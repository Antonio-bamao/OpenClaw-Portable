...
- 当前阶段：`v2026.05.3` 源码侧已定位并修复微信外部 npm 插件 register 失败的根因；本地保留现场 smoke 已证明 gateway 不再报 `Unable to resolve plugin runtime module`，用户在 `2026-05-08` 真实复测后确认“这次终于好了”。微信 register / 连接问题已闭环。
...
- 最新验证：`python -m unittest discover -s tests` 通过 `308` 项测试；`python scripts\verify-wechat-plugin-runtime.py --package-root dist\OpenClaw-Portable --timeout-seconds 45 --post-ready-wait-seconds 3 --output tmp\wechat-plugin-runtime-smoke.json` 返回 `ok=true`。该 smoke 的 stderr 只剩外部插件发现 warning，没有 `failed during register`。
- 防复发重点：下一版给用户的 zip 必须重新打包，不能复用旧 release zip；打包后必须跑 `python scripts\verify-wechat-plugin-runtime.py --package-root dist\OpenClaw-Portable` 覆盖已安装外部 npm 微信插件的 gateway register 路径。
- 上下文维护：`work-log.md` 已超过 400 行，后续流水写入 `work-log（2）.md`；本次异常已在 `bug-log.md` 更新为“已解决并经用户真实复测确认”。
