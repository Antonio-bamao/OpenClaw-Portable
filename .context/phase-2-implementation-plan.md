...
- 部分完成：启动器错误边界已将缺少运行时、启动超时、提前退出映射为中文可理解提示，并指向 `openclaw-runtime.err.log` 供售后排查；真实模式下 API Key 缺失会在主面板显示 Provider 名称与“重新配置”建议；主面板已开始渲染 `status_detail` 并展示运行时长。
- 待补充：首启等待状态、进一步的 Provider 配置诊断。
