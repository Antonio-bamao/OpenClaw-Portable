...
- 已完成：启动器错误边界已将缺少运行时、启动超时、提前退出映射为中文可理解提示，并指向 `openclaw-runtime.err.log` 供售后排查；真实模式下 API Key 缺失会在主面板显示 Provider 名称与“重新配置”建议；主面板已开始渲染 `status_detail` 并展示运行时长；启动/重启时会先切换到“启动中 / 重启中”的等待状态并提示首启可能需要 20-60 秒；自定义 Provider 缺少接口地址或模型名时会明确提示重新配置。
