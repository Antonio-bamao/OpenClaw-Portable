...
## 2026-04-16 / U-disk runtime cache and real removable-media validation

- Goal: finish the highest-priority U-disk delivery loop after a real `D:` removable drive was mounted.
- Actions: rebuilt and signed `dist/OpenClaw-Portable`, copied it to `D:\OpenClaw-Portable`, reproduced the original D-drive runtime failure, traced it to FAT32/removable-media small-file startup cost, and added removable-drive runtime staging. `OpenClawRuntimeAdapter` now detects removable package roots, stages `runtime/openclaw` into a versioned local cache under the launcher temp cache using Windows `robocopy /MT`, and starts OpenClaw from the local cache while keeping `state/` on the U disk. Also hardened startup readiness so one transient socket accept is not enough; the adapter now requires two consecutive successful health checks before declaring the gateway ready.
- Result: the U-disk package is synchronized and audit-clean. The previous D-drive direct startup failure is mitigated by the local runtime cache path, and restart verification is stable after the consecutive-healthcheck fix.
- Verification: `python -m unittest discover -s tests` passed `159` tests. `powershell -ExecutionPolicy Bypass -File .\scripts\build-launcher.ps1` succeeded, then `python scripts\sign-update-manifest.py --package-root dist\OpenClaw-Portable --private-key-path .local\update-signing-private-key.txt` regenerated the signature. `python scripts\audit-portable-package.py --package-root dist\OpenClaw-Portable --top 5` passed at `558.55MB / 25839` files with no warnings. Local delivery gate passed package/release/runtime checks with cold `25.12s`, restart `25.58s`. `robocopy dist\OpenClaw-Portable D:\OpenClaw-Portable /E /R:1 /W:1 /MT:8` copied `172` changed files with `FAILED=0`. D-drive audit passed at `558.55MB / 25839` files, robocopy dry-run returned no remaining differences, and source/D counts matched exactly. D-drive delivery gate with `--cold-runs 1 --restart-runs 1 --removable-media-path D:\OpenClaw-Portable` returned `status=pending` with package audit passed, release assets passed, runtime stability passed (cold `34.72s`, restart `21.72s`), and removable-media evidence passed.
- Remaining: real Feishu private-chat E2E still needs actual Feishu app credentials; multi-engine AV/SmartScreen evidence is still pending. The current `dist\release` assets are still version `v2026.04.2` and were used only as release-asset presence checks, not as a newly cut release containing this runtime-cache fix.

### Final rerun note

- Final verification rerun after context updates: `python -m unittest discover -s tests` passed `159` tests, and `python scripts\verify-delivery-flow.py --package-root D:\OpenClaw-Portable --release-dir dist\release --cold-runs 1 --restart-runs 1 --timeout-seconds 90 --removable-media-path D:\OpenClaw-Portable --output tmp\delivery-flow-gate-usb-runtime-cache-final.json` returned `status=pending` with package audit passed, release assets passed, runtime stability passed (cold `31.20s`, restart `21.70s`), and removable-media evidence passed. Remaining pending checks are external Feishu E2E and multi-engine AV/SmartScreen evidence.
...
## 2026-04-16 / Prepare v2026.04.3 release candidate

- Goal: produce a new local release candidate that includes the remote-backed social-channel work and removable-media runtime-cache fix without reusing the already-published `v2026.04.2` version.
- Actions: bumped `version.json` from `v2026.04.2` to `v2026.04.3`, ran the focused update/release regression slice, rebuilt signed release assets through `scripts\build-release-assets.ps1`, regenerated `update.json` with clean ASCII release notes via `scripts\build-release-assets.py`, ran local delivery flow verification, synced the rebuilt package to `D:\OpenClaw-Portable`, audited the U-disk package, checked robocopy dry-run parity, ran the D-drive delivery gate, and reran the full unittest suite.
...
## 2026-04-18～Add real QQ onboarding and explicit WeChat login confirmation
- 目标：Add real QQ onboarding and explicit WeChat login confirmation
- 动作：Wrote a focused spec and implementation plan for launcher-owned QQ runtime onboarding plus WeChat confirmation; added red-phase tests for QQ onboarding command format, QQ enable behavior, and the WeChat `确认已扫码` action; implemented QQ credential fingerprint persistence, `qq_onboarding_command()`, `onboard_qq_channel()`, controller-side validate/onboard/enable sequencing, `confirm_wechat_channel_login()`, and the new WeChat confirm button plus app handler; then rebuilt the local packaged launcher.
...
## 2026-04-25｜修复首启联调误导文案与 runtime 模板打包缺失
- 目标：处理最新本地联调里两个真实阻塞点：首次向导“开始测试”把占位逻辑说成“已模拟通过连接测试”，以及便携包联调时缺少 `runtime/openclaw/docs/reference/templates/AGENTS.md` 导致控制台报 workspace template 缺失。
- 动作：先用 `rg` 与源码对照确认 `launcher/ui/wizard.py` 中的 `_simulate_connection_test()` 仍是硬编码占位文案，同时确认源码里的 `runtime/openclaw/docs/reference/templates/AGENTS.md` 存在、但 `dist/OpenClaw-Portable` 中缺失，根因是 `launcher/services/runtime_pruning.py` 默认按 `*.md` 一刀切裁剪。随后按 TDD 补了 3 组回归测试：保留 runtime 模板目录、首次向导连接页输出诚实提示、portable audit 不再把保留模板误报成 markdown prune candidate；再实现最小修复：为 runtime pruning 增加 `docs/reference/templates/*.md` 保留规则，把首次向导连接页改成“本页不会直接连接远端，真实探测会在保存配置并启动本地运行时后执行”，并让 `portable_audit` 排除这批必须保留的模板。修复完成后，继续运行 `scripts/build-release-assets.ps1`，把本地 `dist/OpenClaw-Portable`、`dist/release/OpenClaw-Portable-v2026.04.6.zip` 与 `update.json` 一并重建到最新状态。
